...
func GetManifestOCIDescriptor(ctx context.Context, hostname, repository, ref string) (oci.Descriptor, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return oci.Descriptor{}, fmt.Errorf("%v: %s", url, resp.Status)
	}

	header := resp.Header
	mediaType := header.Get("Content-Type")
	if mediaType != schema2.MediaTypeManifest {
		return oci.Descriptor{}, fmt.Errorf("%v: media type mismatch: %s", url, mediaType)
	}
	...
	if err != nil {
		return oci.Descriptor{}, fmt.Errorf("%v: invalid Docker-Content-Digest: %s", url, contentDigest)
	}
